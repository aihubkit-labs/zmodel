...
| `1080p` | 1–10 的整数 | 24 fps | 241 | 最长10秒 |

该表是平台API合同，适用于当前支持的所有 `ratio`。Agnes 上游可能按分辨率和宽高比分别设置帧数
上限；平台对1080p统一采用已确认可用的10秒上限，避免用户因宽高比不同遇到不可预测的上游错误。
超过对应范围时返回 HTTP 400，不创建上游任务。
...
13. Agnes 1080p 在 10 秒时发送 241 帧和 24 fps，11–18 秒在平台侧直接拒绝。
