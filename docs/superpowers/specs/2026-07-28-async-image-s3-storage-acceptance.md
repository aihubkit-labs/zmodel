...
耗时、生成状态、输出状态和详情，其余可选列默认隐藏。切换列时表头、数据单元格和空结果提示
必须保持对齐。
...
回退值。详情弹窗仍应同时展示用户名和用户 ID。

对一个 `available` 任务点击“详情”，确认详情弹窗同时展示图片画廊、预览、下载、生成、计费、归档
重试参数、生命周期和逐对象的上传、暂存、删除状态；宽屏下生成详情、归档处理、计费和生命周期应以
双栏利用弹窗宽度，请求参数跨整行展示。
...
路径。请求参数应显示提交时安全快照中的模型、Prompt、数量和图片生成控制参数，不得包含图片、蒙版、
用户标识、任意扩展字段、Base64 或 data URI。对 `failed`、`expired` 或已删除对象打开详情，预期保留状态和错误信息，但不显示可用的预览、
下载链接。
