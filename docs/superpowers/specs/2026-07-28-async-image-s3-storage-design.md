...
图片消费日志和用户/渠道统计在生成成功结算时产生；生成失败退款在首次从
`billing_status=reserved` 转为 `refunded` 时精确写入一条任务退款日志。该日志使用 `task_id`
作为 `request_id`，包含任务 ID、错误码、脱敏失败原因、请求路径、用户、Token、渠道、模型、
分组、退回额度和耗时；重复退款或 Worker 重复扫描不得产生重复日志。归档失败另写
...
- 列表参考现有任务日志，展示提交时间、结束时间、渠道、用户、任务 ID、分组、平台、模型、耗时、
  ...
  列均可切换。Root 默认显示提交时间、结束时间、用户、分组、渠道、模型、耗时、生成状态和输出
  状态；普通用户默认显示其中有权访问的列，其他运维列
  默认隐藏；普通用户不会在菜单中看到 Root 专属列。列选择按 Root/普通用户分别持久化到浏览器，
  刷新页面后保持不变。任务 ID 在列表中完整展示；`output_availability` 在页面中命名为“上传状态”，
  其中 `available` 显示为“已上传”。生成状态、上传状态和计费状态筛选器未选择时显示各自的条件名称，
  下拉菜单及选中值按当前界面语言显示，发送给接口的查询值仍使用稳定的英文枚举。用户列只显示
  用户名，缺少用户名的历史数据才显示带明确标签的用户 ID。详情弹窗显示用户名和渠道名称，不展示
  用户 ID、渠道 ID 或 Token ID。
- “详情”是图片预览、下载和任务信息的唯一入口，详情弹窗优先展示图片画廊，每张图片提供原图预览
  和下载；单图时对象详情占满可用宽度，多图时使用双栏画廊。宽屏下左栏连续展示生成详情与计费，
  右栏连续展示归档处理与生命周期，避免较短区块下方出现大块空白；请求参数跨整行展示。请求参数
  使用提交时生成的安全快照，保留模型、Prompt、数量和图片生成控制参数，排除图片、蒙版、用户标识、
  任意扩展字段及其可能包含的 Base64/data URI。改造前已结算且原始请求已清空的历史任务无法补建
  快照，页面明确显示“历史任务未保留请求参数”。随后展示逐对象的上传/暂存/删除状态、文件类型、
  文件大小和时间。Root 额外显示 Token、订阅、渠道、Provider、Endpoint、Region、Bucket、Object
  Key、ETag 和内部错误等审计字段；前端不展示用户 ID、渠道 ID 或 Token ID，任何角色都不显示 S3
  凭据或持久暂存相对路径。
...
本需求的可执行手工验收步骤见
[`2026-07-28-async-image-s3-storage-acceptance.md`](./2026-07-28-async-image-s3-storage-acceptance.md)。
